...
class Lock {
  ...
  acquire(key, timeout, retry, retryWait, callback) {
    let self = this;
    self.scriptManager.run('script-acquire', [key, self.value, timeout], [], (e, res) => {
      if (e)
        return callback(e);
      if (res === 0) {
        if (retry > 0) {
          return setTimeout(function () {
            self.acquire(key, timeout, retry - 1, retryWait, callback);
          }, retryWait);
        }
